...
> Saito, H. (2026). Social Isolation and Benzodiazepine Prescribing Disparities in Japan: A Prefecture-Level Ecological Study.
...
- `social_isolation_bz_analysis_dataset.csv` — Main analysis dataset
...
Required files: NDB Open Data No.10 — Drug effect classification section
...
Required: 2020 National Census — Single-person households aged ≥65 years by prefecture
...
# Step 7: Generate all figures (Figure 1–5)
...
```bash
cd 04_Manuscripts
quarto render 09Manuscript_social_isolation_bz_AGG_submission.qmd --to docx
```

Requires Quarto and Pandoc installed. The `apa.csl` and `references.bib` files are included.

## Notes on Reproducibility

- All random seeds are fixed (where applicable)
- The analysis is deterministic given the same input data
- Spatial weights use Queen contiguity + K=2 KNN for island prefectures (Okinawa)
